...
# SCRAPYDWEB_BIND = "192.168.110.103"
...
# from scrapyd.config import Config; path = abspath(Config().get("logs_dir")); print(path); print(isdir(path))"
...
SCHEDULE_ADDITIONAL = "-d setting=CLOSESPIDER_TIMEOUT=600"
...
#   - The SIGTERM signal would be sent only one time to shut down the news_crawler gracefully.
...
DATA_PATH = os.environ.get(
    "DATA_PATH", os.path.join(os.path.dirname(__file__), "data/scrapydweb")
)
...
PG_USER = os.environ.get("PG_USER", "docker")
PG_PASSWORD = os.environ.get("PG_USER", "docker")
PG_HOST = os.environ.get("PG_HOST", "postgres")
PG_PORT = os.environ.get("PG_PORT", "5432")
# PG_DB is not used, because scrapydweb create databases by itself
